...
class SwiftBuildExt(build_ext):
    ...
    def run(self):
        ...
        # Find the built library (check platform-specific and legacy paths)
        if sys.platform == "darwin":
            lib_name = "libPygorilla.dylib"
        else:
            lib_name = "libPygorilla.so"
        ...
        dest_dir = src_dir / "pygorilla" / "_native"
        dest_dir.mkdir(exist_ok=True)
        src_dest = dest_dir / "pygorilla.so"
        ...
        if self.build_lib:
            build_dest = Path(self.build_lib) / "pygorilla" / "_native" / "pygorilla.so"
...
setup(
    ext_modules=[Extension("pygorilla._swift", sources=[])],
